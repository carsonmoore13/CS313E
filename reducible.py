...
def step_size(s):
    ...
    hash_value = hash_word(s, 3)

    step_size_value = hash_value % STEP_SIZE_CONSTANT

    if step_size_value == 0:
        return STEP_SIZE_CONSTANT

    return step_size_value
...
def insert_word(s, hash_table):
    ...
    """
    size = len(hash_table)
    initial_index = hash_word(s, size)
    step = step_size(s)
...
def main():
    ...
    hash_table_size = 2* word_list_length
    n = next_prime(hash_table_size)

    hash_table = [""] * n
    for word in word_list:
        insert_word(word, hash_table)

    hash_memo_factor = 0.2
    hash_memo_size = next_prime(int(hash_memo_factor * word_list_length))
