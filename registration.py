...
On my/our honor, <CARSON MOORE>, this
...
class Stack:
    ...
    def peek(self):
        '''This is the method that completes the peek() operation'''
        if self.is_empty():
            raise StackError("Peek from empty stack.")
        return self._top.data

    def push(self, item):
        '''This is the method that defines the push() operation.'''
        new_node = Node(item)
    ...
    def pop(self):
        '''The method creates the functionality to be able to pop().'''
        if self.is_empty():
            ...
            for i in range(len(temp_vertices)):
                ...
                for j in range(len(temp_vertices)):
                    if temp_matrix[j][i] != 0:
                        ...
                        break
                if is_ready:
                    semester.append(temp_vertices[i].label)
                    to_delete.append(i)

            courses.append(semester[:4])

            for index in sorted(to_delete[:4], reverse = True):
...
def main():
    ...
    for _ in range(num_edges):
        ...
        from_vertex, to_vertex = edge.split()
        from_index = graph.get_index(from_vertex)
        to_index = graph.get_index(to_vertex)
